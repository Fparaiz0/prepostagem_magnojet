...
@section('content')
    ...
    <div class="content-header">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            @can('edit-recipient')
            <a href="{{ route('recipients.edit', ['recipient' => $recipient->id]) }}" 
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-gray-800 font-medium">{{ $recipient->name }}</p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">CNPJ</p>
                            <p class="text-gray-800 font-mono">{{ $recipient->cnpj }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-5">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p class="text-gray-800">
                            ...
                            {{ \Carbon\Carbon::parse($recipient->created_at)->format('H:i') }}
                        </p>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p class="text-gray-800">
                            {{ \Carbon\Carbon::parse($recipient->updated_at)->format('d/m/Y') }}
                            <span class="text-gray-400 mx-2">•</span>
                            {{ \Carbon\Carbon::parse($recipient->updated_at)->format('H:i') }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="px-8 py-6 border-t border-gray-100 bg-gray-50 flex justify-end">
            <form action="{{ route('recipients.destroy', ['recipient' => $recipient->id]) }}" method="POST">
                ...
                @method('delete')
                <button type="submit" 
                        onclick="return confirm('Tem certeza que deseja excluir permanentemente este destinatário?')" 
                        class="flex items-center px-5 py-2.5 bg-white border border-red-200 rounded-xl text-red-600 hover:bg-red-50 transition-all shadow-xs hover:shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
