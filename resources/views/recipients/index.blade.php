...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <form action="{{ route('recipients.index') }}" class="space-y-4 md:space-y-0 md:grid md:grid-cols-12 md:gap-6 md:items-end">
    ...
    <div class="bg-gray-50 rounded-lg shadow-sm overflow-hidden border border-blue-200">
        ...
        <div class="grid grid-cols-12 divide-x divide-blue-200 border-b border-blue-200 hover:bg-white transition-colors">
    ...
    <div class="mt-6 px-4 py-3 flex flex-col sm:flex-row items-center justify-between bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex items-center space-x-1">
            ...
            @else
                <a href="{{ $recipients->url(1) }}" class="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">
                    &laquo;
                </a>
                <a href="{{ $recipients->previousPageUrl() }}" class="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">
                    &lsaquo;
                </a>
            @endif
            ...
            @foreach ($recipients->getUrlRange(max(1, $recipients->currentPage() - 2), min($recipients->lastPage(), $recipients->currentPage() + 2)) as $page => $url)
                ...
                @else
                    <a href="{{ $url }}" class="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">
                        {{ $page }}
                    </a>
                @endif
            @endforeach
            ...
            @if ($recipients->hasMorePages())
                <a href="{{ $recipients->nextPageUrl() }}" class="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">
                    &rsaquo;
                </a>
                <a href="{{ $recipients->url($recipients->lastPage()) }}" class="px-3 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors">
                    &raquo;
                </a>
            @else
                <span class="px-3 py-2 rounded-lg border border-gray-300 text-gray-400 cursor-not-allowed">
