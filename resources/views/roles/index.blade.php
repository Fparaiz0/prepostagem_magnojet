...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden mt-6 border border-blue-200">
                    ...
                    <div class="flex items-center space-x-2 flex-wrap gap-2">
                        ...
                        <a href="{{ route('role-permissions.index', ['role' => $role->id]) }}" 
                           ...
                           <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                             <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c.828 0 1.5.672 1.5 1.5S12.828 14 12 14s-1.5-.672-1.5-1.5S11.172 11 12 11zm4-6a4 4 0 10-8 0v4h8V5zM6 9v10h12V9H6z" />
                           </svg>
                            Permissões
                        </a>
                        @endcan
                        ...
                        @can('destroy-role')
                        <form action="{{ route('roles.destroy', ['role' => $role->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('delete')
                            <button type="submit" 
        ...
        <div class="px-6 py-12 text-center">
            ...
            @can('create-role')
            <a href="{{ route('roles.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
