...
@section('content')
    ...
    <div class="content-header">
        <h2 class="content-title">Editar Status de Usuário</h2>
        ...
        <form action="{{ route('user_statuses.update', ['userStatus' => $userStatus->id]) }}" method="POST">
            ...
            <div class="p-6">
                ...
                <div class="pb-5 border-b border-blue-200">
                    <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                        ...
                        Editar Informações do Status
                    </h3>
                    <p class="text-sm text-gray-500 mt-2">Atualize o nome do status de usuário</p>
                </div>

                <!-- Informações do Registro -->
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 border border-blue-200 p-4 bg-gray-50 rounded-lg">
                    <div class="space-y-1">
                        <label class="block text-xs font-medium text-gray-500">ID do Registro</label>
                        <p class="text-sm font-medium text-gray-800">{{ $userStatus->id }}</p>
                    </div>
                    <div class="space-y-1">
                        <label class="block text-xs font-medium text-gray-500">Data de Criação</label>
                        <p class="text-sm text-gray-800">{{ \Carbon\Carbon::parse($userStatus->created_at)->format('d/m/Y H:i:s') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 space-y-2 max-w-md">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $userStatus->name) }}"
                           class="w-full px-4 py-2.5 border border-blue-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div class="px-6 py-4 border-t border-blue-200 bg-gray-50 flex justify-between">
                ...
                <div class="flex space-x-3">
                    ...
                    <button type="submit" 
                            class="px-6 py-2 bg-indigo-600 cursor-pointer text-white font-medium rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
