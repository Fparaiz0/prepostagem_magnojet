...
@section('content')
    ...
    <div class="content-header">
        <h2 class="content-title">Detalhes do Status</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-6">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800">Informações do Status</h3>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $userStatus->id }}</p>
                </div>

                <!-- Nome -->
                <div class="space-y-2">
                    <label class="block text-sm font-medium text-gray-500">Nome do Status</label>
                    <p class="text-lg font-semibold text-indigo-600">{{ $userStatus->name }}</p>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-sm text-gray-800 flex items-center">
                        ...
                        {{ \Carbon\Carbon::parse($userStatus->created_at)->format('d/m/Y H:i:s') }}
                    </p>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-sm text-gray-800 flex items-center">
                        ...
                        {{ \Carbon\Carbon::parse($userStatus->updated_at)->format('d/m/Y H:i:s') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end">
            ...
            <form action="{{ route('user_statuses.destroy', ['userStatus' => $userStatus->id]) }}" method="POST">
                ...
                @method('delete')
                <button type="submit" 
                        onclick="return confirm('Tem certeza que deseja excluir permanentemente este status?')"
                        class="px-4 py-2 bg-red-100 cursor-pointer text-red-700 rounded-lg hover:bg-red-200 transition-colors flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <a href="{{ route('user_statuses.index') }}" 
               class="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center">
                ...
                <div>
                    <h4 class="font-medium text-gray-800">Ver Todos os Status</h4>
                    <p class="text-sm text-gray-500 mt-1">Lista completa de status</p>
                </div>
            </a>
            @endcan
            
            @can('create-user-status')
            <a href="{{ route('user_statuses.create') }}" 
               class="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center">
                ...
                <div>
                    <h4 class="font-medium text-gray-800">Criar Novo Status</h4>
                    <p class="text-sm text-gray-500 mt-1">Adicionar outro status</p>
                </div>
            </a>
            @endcan
        </div>
