...
@section('content')
    ...
    <div class="content-header">
        ...
        <a href="{{ route('packagings.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-6 border border-blue-200">
        @if($packagings->count() > 0)
            ...
            <div class="px-6 py-4 border-t border-blue-200 bg-gray-50 flex flex-col sm:flex-row items-center justify-between">
                <div class="text-sm text-gray-600 mb-2 sm:mb-0">
                    Mostrando <span class="font-medium">{{ $packagings->firstItem() }}</span> a <span class="font-medium">{{ $packagings->lastItem() }}</span> de <span class="font-medium">{{ $packagings->total() }}</span> resultados
                </div>
                <div class="flex items-center space-x-1">
                    ...
                    @else
                        <a href="{{ $packagings->url(1) }}" class="px-3 py-1 rounded border border-gray-200 hover:bg-gray-50 transition-colors">
                            &laquo;
                        </a>
                        <a href="{{ $packagings->previousPageUrl() }}" class="px-3 py-1 rounded border border-gray-200 hover:bg-gray-50 transition-colors">
                            &lsaquo;
                        </a>
                    @endif
                    ...
                    @foreach ($packagings->getUrlRange(max(1, $packagings->currentPage() - 2), min($packagings->lastPage(), $packagings->currentPage() + 2)) as $page => $url)
                        @if ($page == $packagings->currentPage())
                            <span class="px-3 py-1 rounded border border-indigo-200 bg-indigo-50 text-indigo-600 font-medium">
                                {{ $page }}
                            </span>
                        @else
                    ...
                    @if ($packagings->hasMorePages())
                        <a href="{{ $packagings->nextPageUrl() }}" class="px-3 py-1 rounded border border-gray-200 hover:bg-gray-50 transition-colors">
                            &rsaquo;
                        </a>
                        <a href="{{ $packagings->url($packagings->lastPage()) }}" class="px-3 py-1 rounded border border-gray-200 hover:bg-gray-50 transition-colors">
                            &raquo;
                        </a>
                    @else
                        <span class="px-3 py-1 rounded border border-gray-200 text-gray-400 cursor-not-allowed">
