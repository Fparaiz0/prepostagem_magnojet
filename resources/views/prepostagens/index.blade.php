...
@section('content')
    ...
    @can('canceled-prepostagem')
        <a href="{{ route('prepostagens.canceled') }}">Listar Pré-Postagens - Canceladas</a><br>
    @endcan
    ...
    @forelse ($prepostagens as $prepostagem)
        ...
        @can('destroy-prepostagem')
            <form action="{{ route('prepostagens.destroy', ['prepostagem' => $prepostagem->id]) }}" method="POST">
                ...
                <button type="submit" onclick="return confirm('Tem certeza que deseja cancelar esta pré-postagem?')">Cancelar</button>
