...
<div class="content-wrapper">
    ...
    <div class="content-header flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Visualize as pré-postagens que foram canceladas</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-red-300 p-6 mb-6">
        ...
        <form action="{{ route('prepostagens.canceled') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-grow">
                ...
                <div class="relative">
                    ...
                    <input 
                        ...
                        class="pl-10 w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors"
                    />
                </div>
            </div>
            ...
            <div class="flex space-x-2">
                <button 
                    type="submit" 
                    class="px-6 py-2.5 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors flex items-center justify-center"
                >
    ...
    <div class="space-y-4">
        @forelse ($prepostagens as $prepostagem)
            <div class="bg-white rounded-xl shadow-sm border border-red-300 p-6 hover:shadow-md transition duration-200">
                ...
                <div class="flex justify-end space-x-3 border-t pt-4 mt-4">
                    @can('show-prepostagem')
                        <a href="{{ route('prepostagens.show', ['prepostagem' => $prepostagem->id]) }}" 
                           class="flex items-center px-4 py-2 border border-red-300 rounded-lg text-red-700 hover:bg-red-50 transition duration-200 text-sm font-medium">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
                ...
                <h3 class="text-xl font-semibold text-gray-700 mb-2">
                    @if(request()->has('search') && !empty(request('search')))
                        Nenhuma pré-postagem cancelada encontrada para "{{ request('search') }}"
                    @else
                        Nenhuma pré-postagem cancelada encontrada
                    @endif
                </h3>
                <p class="text-gray-500 mb-6">
                    ...
                    @else
                        Não há pré-postagens canceladas no momento.
                    @endif
                </p>
            </div>
        @endforelse
                    ...
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        ...
                        @foreach ($prepostagens->getUrlRange(max(1, $prepostagens->currentPage() - 2), min($prepostagens->lastPage(), $prepostagens->currentPage() + 2)) as $page => $url)
                            @if ($page == $prepostagens->currentPage())
                                <span aria-current="page" class="z-10 bg-red-50 border-red-500 text-red-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
                                    {{ $page }}
                                </span>
                            @else
