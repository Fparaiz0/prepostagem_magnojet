...
@section('content')
    ...
    <div class="content-header flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Controle as permissões atribuídas ao papel: <span
                    class="font-semibold text-purple-600">{{ $role->name }}</span></p>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm font-medium text-gray-800">{{ count($rolePermissions ?? []) }} de
                        {{ $permissions->count() }}</p>
                </div>
            </div>
        </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-4 mt-1">
                                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">ID:
                                            {{ $permission->id }}</span>
                                        @can('create-permission')
                                            <span
                                                class="text-xs font-mono bg-blue-100 text-blue-600 px-2 py-1 rounded">{{ $permission->name }}</span>
                                        @endcan
                                        <span class="text-xs bg-purple-100 text-purple-600 px-2 py-1 rounded">Papel:
                                            {{ $role->name }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="shrink-0">
                            @if (in_array($permission->id, $rolePermissions ?? []))
                                <a href="{{ route('role-permissions.update', ['role' => $role->id, 'permission' => $permission->id]) }}"
                                    ...
                                    Liberado
                                    <span class="ml-2 text-xs text-green-500 group-hover:text-green-700">(Clique para
                                        bloquear)</span>
                                </a>
                            @else
                                <a href="{{ route('role-permissions.update', ['role' => $role->id, 'permission' => $permission->id]) }}"
                                    class="flex items-center px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors group">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-red-600" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                    Bloqueado
                                    <span class="ml-2 text-xs text-red-500 group-hover:text-red-700">(Clique para
                                        liberar)</span>
                                </a>
                            @endif
                        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $permissions->count() - count($rolePermissions ?? []) }}
                    </p>
                </div>
            </div>
        </div>
